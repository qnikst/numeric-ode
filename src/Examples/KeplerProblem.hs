...
data T3 = T3 !Double !Double !Double deriving (Eq,Show)
data T2 = T2 !T3 !T3 deriving (Eq,Show)

instance AdditiveGroup T3 where
    zeroV = T3 zeroV zeroV zeroV
    negateV (T3 x y z) = T3 (negateV x) (negateV y) (negateV z)
    (T3 x1 y1 z1) ^+^ (T3 x2 y2 z2) = T3 (x1^+^x2) (y1^+^y2) (z1^+^z2)

instance VectorSpace T3 where
    type Scalar T3 = Double
    c *^ (T3 x y z) = T3 (c*^x) (c*^y) (c*^z)

instance InnerSpace T3 where
    (T3 x1 y1 z1) <.> (T3 x2 y2 z2) = x1*x2+y1*y2+z1*z2
...
result1 = runST $ integrateV (stormerVerlet2 kepler) ((T2 (T3 0.1 0 0) (T3 (-0.1) 0 0)),(T2 (T3 (-1) (-1) 0) (T3 1 1 0))) tm
result2 = runST $ integrateV (k2p) ((T2 (T3 0.1 0 0) (T3 (-0.1) 0 0)),(T2 (T3 (-1) (-1) 0) (T3 1 1 0))) tm
...
plot1 r = renderableToPNGFile (mychart) 640 480 "all.png"
    where
        ...
        plot1  = plot_lines_style .> line_color ^= opaque blue
               ...
               $ plot_lines_values ^= [map (\(_,T2 (T3 x y z) _) -> (x,y))  (V.toList r)]
               $ defaultPlotLines
        plot2  = plot_lines_style .> line_color ^= opaque red
               $ plot_lines_title ^= "body-2"
               $ plot_lines_values ^= [map (\(_,T2 _ (T3 x y z)) -> (x,y))  (V.toList r)]
               $ defaultPlotLines
